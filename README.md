# 🎯 Nutsa Gobegishvili | QA Portfolio

<div align="center">

### 💼 Junior QA Tester | Manual & Exploratory Testing Specialist

*Passionate about quality, detail-oriented, and committed to delivering bug-free user experiences*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/nutsa-gobejishvili-1b5779333)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail)](mailto:cucagobejishvili@gmail.com)

</div>

---

## 👋 About Me

Hi! I'm **Nutsa**, a motivated and detail-oriented QA Tester with theoretical and practical training in software testing. I have hands-on experience working on real projects, focusing on improving product quality and reliability through systematic testing approaches.

This portfolio showcases my testing work, including **test cases**, **bug reports**, and **QA documentation** that demonstrate my ability to identify issues and ensure smooth user experiences.

---

## 🚀 What I Do

✅ **Create and manage comprehensive test cases**  
✅ **Write detailed bug reports** (steps-to-reproduce, severity, priority)  
✅ **Develop QA documentation** (test strategies, checklists)  
✅ **Perform Functional, Regression, and Exploratory testing**  
✅ **Collaborate with development teams** to ensure quality delivery

---

## 🛠️ Tools & Technologies

- 📋 **Trello** - Task and project management
- 🧪 **QASE.io** - Test case management
- 📊 **Jira** - Bug tracking and project management  
- 💻 **Visual Studio Code** - Development and scripting

---

## 📊 Testing Expertise

| **Testing Type** | **Skills** |
|------------------|------------|
| 🔍 **Manual Testing** | Functional, UI/UX, Compatibility Testing |
| 🔄 **Regression Testing** | Ensuring stability after changes |
| 🧭 **Exploratory Testing** | Finding unexpected bugs through creative scenarios |
| 📋 **Test Documentation** | Test plans, test cases, checklists, reports |

---

## 📁 Portfolio Structure

### 📂 Test Documentation

> *Comprehensive testing materials organized for easy navigation*

#### 🗂️ [Test_Cases](./Test_Cases)
- ✅ Excel format test cases
- ✅ Qase.io exports
- ✅ Markdown documentation

#### 🗂️ [Test_Scenarios](./Test_Scenarios)
- High-level test scenarios
- User flow testing documentation

#### 🗂️ [Checklists](./Checklists)
- 🔥 Smoke Testing Checklists
- 🔄 Regression Testing Checklists
- ✔️ Sanity Testing Checklists
- 🎨 Usability Testing Checklists

---

### 📂 Bug Reports

> *Detailed bug documentation with reproducible steps*

#### 🐛 [Bug_Reports](./Bug_Reports)
- Severity and priority classifications
- Step-by-step reproduction steps
- Screenshots and evidence
- Status tracking and resolution notes

---

### 📂 Learning & Development

> *Continuous improvement and skill development*

#### 📚 [Learning](./Learning)
- Testing methodologies and best practices
- Tools and frameworks documentation
- Study materials and resources
- Personal notes and insights

---

### 📂 Certificates & Achievements

> *Professional certifications and milestones*

#### 🏆 [Certificates](./Certificates)
- QA testing certifications
- Training completion certificates
- Professional development achievements

---

### 📂 Test Artifacts

> *Technical competency demonstrations*

#### 🔌 [API_Testing](./API_Testing)
- Postman collections
- API test scenarios
- Request/Response examples
- Screenshots of test results

#### 💾 [SQL_Queries](./SQL_Queries)
- Database testing queries
- Data validation scripts
- SQL test examples

#### ♿ [Accessibility_UI_Review](./Accessibility_UI_Review)
- UI/UX review reports
- Accessibility testing results
- WCAG compliance checks
- Screenshots and findings

---

## 📈 My Approach to Testing
```
1. 📖 Understand Requirements  
2. 📝 Design Test Cases  
3. 🔍 Execute Tests  
4. 🐛 Report Bugs  
5. ✅ Verify Fixes  
6. 📊 Document Results
```

---

## 💡 Why Work With Me?

- ✨ **Detail-Oriented**: I catch bugs others miss
- 🎯 **Systematic Approach**: Organized and methodical testing process
- 📚 **Continuous Learner**: Always improving my testing skills
- 🤝 **Team Player**: Collaborative and communicative
- 🚀 **Passionate**: Genuinely excited about quality assurance

---

## 📫 Let's Connect!

I'm actively looking for **Junior QA Tester** opportunities where I can contribute to building quality software.

<div align="center">

### 🔗 Find Me Online

[![LinkedIn] https://www.linkedin.com/in/nutsa-gobejishvili-1b5779333/
[![Email](https://img.shields.io/badge/Email-cucagobejishvili@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:cucagobejishvili@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-View_My_Work-success?style=for-the-badge&logo=github)](https://github.com/NutsaGobe/Nutsa-s-Portfolio)

---

### ⭐ If you like my work, please consider giving this repository a star!

</div>

---

<div align="center">
<sub>Last updated: November 2024</sub>
</div>

# 🎯 Nutsa Gobegishvili | QA Portfolio

<div align="center">

### 💼 Junior QA Tester | Manual & Exploratory Testing Specialist

*Passionate about quality, detail-oriented, and committed to delivering bug-free user experiences*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/nutsa-gobejishvili-1b5779333)
[![Email](https://img.shields.io/badge/Email-Contact-red?style=for-the-badge&logo=gmail)](mailto:cucagobejishvili@gmail.com)

</div>

---

## 👋 About Me

Hi! I'm **Nutsa**, a motivated and detail-oriented QA Tester with theoretical and practical training in software testing. I have hands-on experience working on real projects, focusing on improving product quality and reliability through systematic testing approaches.

This portfolio showcases my testing work, including **test cases**, **bug reports**, and **QA documentation** that demonstrate my ability to identify issues and ensure smooth user experiences.

---

## 🚀 What I Do

✅ **Create and manage comprehensive test cases**  
✅ **Write detailed bug reports** (steps-to-reproduce, severity, priority)  
✅ **Develop QA documentation** (test strategies, checklists)  
✅ **Perform Functional, Regression, and Exploratory testing**  
✅ **Collaborate with development teams** to ensure quality delivery

---

## 🛠️ Tools & Technologies

<table>
<tr>
<td align="center" width="150">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/trello/trello-plain.svg" width="48" height="48" alt="Trello" />
<br><b>Trello</b>
</td>
<td align="center" width="150">
<img src="https://avatars.githubusercontent.com/u/13406071?s=200&v=4" width="48" height="48" alt="Qase" />
<br><b>QASE.io</b>
</td>
<td align="center" width="150">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/jira/jira-original.svg" width="48" height="48" alt="Jira" />
<br><b>Jira</b>
</td>
<td align="center" width="150">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="48" height="48" alt="VS Code" />
<br><b>VS Code</b>
</td>
</tr>
</table>

---

## 📊 Testing Expertise

| **Testing Type** | **Skills** |
|------------------|------------|
| 🔍 **Manual Testing** | Functional, UI/UX, Compatibility Testing |
| 🔄 **Regression Testing** | Ensuring stability after changes |
| 🧭 **Exploratory Testing** | Finding unexpected bugs through creative scenarios |
| 📋 **Test Documentation** | Test plans, test cases, checklists, reports |

---

## 📁 Portfolio Structure

### 📂 Test Documentation

> *Comprehensive testing materials organized for easy navigation*

#### 🗂️ [Test_Cases](./Test_Cases)
- ✅ Excel format test cases
- ✅ Qase.io exports
- ✅ Markdown documentation

#### 🗂️ [Test_Scenarios](./Test_Scenarios)
- High-level test scenarios
- User flow testing documentation

#### 🗂️ [Checklists](./Checklists)
- 🔥 Smoke Testing Checklists
- 🔄 Regression Testing Checklists
- ✔️ Sanity Testing Checklists
- 🎨 Usability Testing Checklists

---

### 📂 Bug Reports

> *Detailed bug documentation with reproducible steps*

#### 🐛 [Bug_Reports](./Bug_Reports)
- Severity and priority classifications
- Step-by-step reproduction steps
- Screenshots and evidence
- Status tracking and resolution notes

---

### 📂 Learning & Development

> *Continuous improvement and skill development*

#### 📚 [Learning](./Learning)
- Testing methodologies and best practices
- Tools and frameworks documentation
- Study materials and resources
- Personal notes and insights

---

### 📂 Certificates & Achievements

> *Professional certifications and milestones*

#### 🏆 [Certificates](./Certificates)
- QA testing certifications
- Training completion certificates
- Professional development achievements

---

### 📂 Test Artifacts

> *Technical competency demonstrations*

#### 🔌 [API_Testing](./API_Testing)
- Postman collections
- API test scenarios
- Request/Response examples
- Screenshots of test results

#### 💾 [SQL_Queries](./SQL_Queries)
- Database testing queries
- Data validation scripts
- SQL test examples

#### ♿ [Accessibility_UI_Review](./Accessibility_UI_Review)
- UI/UX review reports
- Accessibility testing results
- WCAG compliance checks
- Screenshots and findings

---

## 📈 My Approach to Testing
```
1. 📖 Understand Requirements  
2. 📝 Design Test Cases  
3. 🔍 Execute Tests  
4. 🐛 Report Bugs  
5. ✅ Verify Fixes  
6. 📊 Document Results
```

---

## 💡 Why Work With Me?

- ✨ **Detail-Oriented**: I catch bugs others miss
- 🎯 **Systematic Approach**: Organized and methodical testing process
- 📚 **Continuous Learner**: Always improving my testing skills
- 🤝 **Team Player**: Collaborative and communicative
- 🚀 **Passionate**: Genuinely excited about quality assurance

---

## 📫 Let's Connect!

I'm actively looking for **Junior QA Tester** opportunities where I can contribute to building quality software.

<div align="center">

### 🔗 Find Me Online

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Nutsa_Gobegishvili-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/nutsa-gobejishvili-1b5779333)
[![Email](https://img.shields.io/badge/Email-cucagobejishvili@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:cucagobejishvili@gmail.com)
[![Portfolio](https://img.shields.io/badge/Portfolio-View_My_Work-success?style=for-the-badge&logo=github)](https://github.com/NutsaGobe/Nutsa-s-Portfolio)

---

### ⭐ If you like my work, please consider giving this repository a star!

</div>

---

<div align="center">
<sub>Last updated: November 2024</sub>
</div>
